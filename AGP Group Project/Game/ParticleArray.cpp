...
//mvStack.push(mvStack.top());
//// pass modelview & projection as uniforms into shader
//rt3d::setUniformMatrix4fv(particleProgram, "modelview", glm::value_ptr(mvStack.top()));
//rt3d::setUniformMatrix4fv(particleProgram, "projection", glm::value_ptr(projection));
//glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
//glEnable(GL_BLEND);
//glDepthMask(0); //stop updates to the depth mask when rendering the particles - This stops them drawing on top of each other.
//particles->draw();
//glDisable(GL_BLEND);
//glDepthMask(1);
//// Update particles
