...
void HUD::draw(stack<glm::mat4> mvStack, GLuint shaderProgram)
{
	...
	for (int i = 1; i < 9; i++)
	{
		...
		mvStack.top() = glm::translate(mvStack.top(), glm::vec3(-0.675f, (-1.1f + (0.2*i)), 0.0f));
		mvStack.top() = glm::scale(mvStack.top(), glm::vec3(0.3, 0.1f, 0.0f));
	...
	label[1] = textToTexture(key1.c_str(), label[1], { 0,255,0 });
	label[2] = textToTexture(key2.c_str(), label[2], { 0,255,0 });
	label[3] = textToTexture(key3.c_str(), label[3], { 0,255,0 });
	label[4] = textToTexture(key4.c_str(), label[4], { 0,255,0 });
	label[5] = textToTexture(key5.c_str(), label[5], { 0,255,0 });
	label[6] = textToTexture(key6.c_str(), label[6], { 0,255,0 });
	label[7] = textToTexture(key7.c_str(), label[7], { 0,255,0 });
	label[8] = textToTexture(key8.c_str(), label[8], { 0,255,0 });
}
...
void HUD::draw(stack<glm::mat4> mvStack, int value, GLuint shaderProgram, string labelText ) 
{
	glUseProgram(shaderProgram); //Use texture-only shader for text rendering
	glDisable(GL_DEPTH_TEST); //Disable depth test for HUD label
	...
	string hudOutput = (labelText + " " + to_string(value));

	label[0] = textToTexture(hudOutput.c_str() , label[0], { 255,0,0 });// calls textToTexture giving the string, the label it is to be placed on and the colour
}
...
void HUD::draw(stack<glm::mat4> mvStack, GLuint shaderProgram, GLuint labelNumber, vec3 labelPosition, vec3 labelScale, SDL_Color labelColor)
{
	...
	string hudOutput = (" Jack D Smith & Greg Smith");
