...
void SceneObjects::draw(stack<glm::mat4> passedStack, GLuint shaderProgram, mat4 projectionMatrix, GLfloat rotation, rt3d::lightStruct light) // Used for drawing the keycards
{
	if (isDrawn)
	{
		...
		mvStack.top() = glm::rotate(mvStack.top(), float(rotation*DEG_TO_RADIAN), glm::vec3(0.0f, 1.0f, 0.0f));
...
void SceneObjects::drawWithTwoTexturesAndTwoLights(stack<glm::mat4> passedStack, GLuint shaderProgram, mat4 projectionMatrix, bool twoTextures, int textureVisible, int specularValue, GLfloat rotation, rt3d::lightStruct light0, rt3d::lightStruct light1, int lightOn)
{
	stack<glm::mat4> mvStack = passedStack;
	glUseProgram(shaderProgram); //use shader program for shading
	rt3d::setUniformMatrix4fv(shaderProgram, "projection", glm::value_ptr(projectionMatrix));
	...
	//std::cout << "lightOn: " << lightOn << std::endl;
...
void SceneObjects::drawWithVariableTextures(stack<glm::mat4> passedStack, GLuint shaderProgram, mat4 projectionMatrix, int texturesCount, int textureVisible, int specularValue, GLfloat rotation, rt3d::lightStruct light0, rt3d::lightStruct light1, vec3 eye, mat4 view)
{
	...
	//Moves the cube to its desired position after it has been scaled
	mvStack.top() = glm::translate(mvStack.top(), objectPos);
	//Rotates the cube after it is scaled
	mvStack.top() = glm::rotate(mvStack.top(), float(rotation*DEG_TO_RADIAN), glm::vec3(0.0f, -1.0f, 0.0f));
	//Scales the cube first
	mvStack.top() = glm::scale(mvStack.top(), objectSize);
...
void SceneObjects::update(vec3 objectPosition, GLfloat rotation)
{
	rotationAmount = rotation;
	objectPos = objectPosition;
	vec3 p1 = vec3((objectPosition.x - objectSize.x), (objectPosition.y + objectSize.y), (objectPosition.z + objectSize.z));
	// Sets the minimum coords for the bounding box
	vec3 p2 = vec3((objectPosition.x + objectSize.x), (objectPosition.y - objectSize.y), (objectPosition.z - objectSize.z));
	// Sets the maximum coords for the bounding box
	aabb = CPM_GLM_AABB_NS::AABB(p1, p2);
}
